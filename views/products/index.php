<?php declare(strict_types=1); ?>

<?php $pageTitle = 'จัดการสินค้า'; ?>

<!-- ╔══════════════════════════════════════════════════════════╗ -->
<!-- ║  Page Header                                             ║ -->
<!-- ╚══════════════════════════════════════════════════════════╝ -->
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div>
        <h4 class="mb-1 fw-bold">
            <i class="bi bi-box-seam me-2 text-primary"></i>จัดการสินค้า
        </h4>
        <p class="text-muted mb-0 small">
            สินค้าทั้งหมด <?= number_format(count($products)) ?> รายการ
        </p>
    </div>
    <a href="<?= url(['page' => 'products', 'action' => 'create']) ?>"
       class="btn btn-primary d-flex align-items-center gap-2">
        <i class="bi bi-plus-circle-fill"></i>
        <span>เพิ่มสินค้าใหม่</span>
    </a>
</div>

<!-- ╔══════════════════════════════════════════════════════════╗ -->
<!-- ║  Summary Cards                                           ║ -->
<!-- ╚══════════════════════════════════════════════════════════╝ -->
<?php
$overallTotalQty    = array_sum(array_column($products, 'total_qty'));
$overallTotalAmount = array_sum(array_column($products, 'total_amount'));
$overallWt1Qty      = array_sum(array_column($products, 'wt1_qty'));
$overallWt1Amount   = array_sum(array_column($products, 'wt1_amount'));
$overallWt2Qty      = array_sum(array_column($products, 'wt2_qty'));
$overallWt2Amount   = array_sum(array_column($products, 'wt2_amount'));
?>
<div class="row g-3 mb-4">

    <!-- Total products -->
    <div class="col-6 col-lg-3">
        <div class="card summary-card border-primary h-100">
            <div class="card-body py-3 px-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <p class="text-muted small mb-1">สินค้าทั้งหมด</p>
                        <h4 class="fw-bold mb-0 text-amount"><?= number_format(count($products)) ?></h4>
                        <small class="text-muted">รายการ</small>
                    </div>
                    <div class="fs-2 text-primary opacity-50">
                        <i class="bi bi-box-seam-fill"></i>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Total qty sold -->
    <div class="col-6 col-lg-3">
        <div class="card summary-card border-info h-100">
            <div class="card-body py-3 px-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <p class="text-muted small mb-1">ขายรวมทั้งหมด</p>
                        <h4 class="fw-bold mb-0 text-amount"><?= number_format($overallTotalQty) ?></h4>
                        <small class="text-muted">ชิ้น</small>
                    </div>
                    <div class="fs-2 text-info opacity-50">
                        <i class="bi bi-stack"></i>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Work type 1 amount -->
    <div class="col-6 col-lg-3">
        <div class="card summary-card border-secondary h-100">
            <div class="card-body py-3 px-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <p class="text-muted small mb-1">ขายเฉพาะเครื่อง</p>
                        <h5 class="fw-bold mb-0 text-amount"><?= formatMoney($overallWt1Amount) ?></h5>
                        <small class="text-muted"><?= number_format($overallWt1Qty) ?> ชิ้น</small>
                    </div>
                    <div class="fs-2 opacity-50" style="color:#0d6efd">
                        <i class="bi bi-cpu-fill"></i>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Work type 2 amount -->
    <div class="col-6 col-lg-3">
        <div class="card summary-card border-success h-100">
            <div class="card-body py-3 px-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <p class="text-muted small mb-1">ขายพร้อมติดตั้ง</p>
                        <h5 class="fw-bold mb-0 text-amount"><?= formatMoney($overallWt2Amount) ?></h5>
                        <small class="text-muted"><?= number_format($overallWt2Qty) ?> ชิ้น</small>
                    </div>
                    <div class="fs-2 text-success opacity-50">
                        <i class="bi bi-tools"></i>
                    </div>
                </div>
            </div>
        </div>
    </div>

</div>

<!-- ╔══════════════════════════════════════════════════════════╗ -->
<!-- ║  Products Table                                          ║ -->
<!-- ╚══════════════════════════════════════════════════════════╝ -->
<div class="card">
    <div class="card-header bg-white d-flex align-items-center justify-content-between py-2 px-3">
        <span class="fw-semibold">
            <i class="bi bi-list-ul me-1"></i>รายการสินค้า
        </span>
        <a href="<?= url(['page' => 'products', 'action' => 'create']) ?>"
           class="btn btn-sm btn-outline-primary">
            <i class="bi bi-plus me-1"></i>เพิ่มสินค้า
        </a>
    </div>

    <?php if (empty($products)): ?>
        <!-- Empty state -->
        <div class="card-body text-center py-5">
            <i class="bi bi-box text-muted" style="font-size:3rem"></i>
            <p class="text-muted mt-3 mb-1 fw-semibold">ยังไม่มีสินค้าในระบบ</p>
            <p class="text-muted small mb-3">เริ่มต้นด้วยการเพิ่มสินค้าชิ้นแรก</p>
            <a href="<?= url(['page' => 'products', 'action' => 'create']) ?>"
               class="btn btn-primary btn-sm">
                <i class="bi bi-plus-circle me-1"></i>เพิ่มสินค้าใหม่
            </a>
        </div>

    <?php else: ?>
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-dark">
                    <tr>
                        <th class="text-center" style="width:50px">#</th>
                        <th>ชื่อสินค้า</th>
                        <th style="width:150px">หมวดหมู่</th>
                        <!-- Work type 1 -->
                        <th class="text-center" colspan="2" style="background-color:#0b5ed7;">
                            <i class="bi bi-cpu me-1"></i>ขายเฉพาะเครื่อง
                        </th>
                        <!-- Work type 2 -->
                        <th class="text-center" colspan="2" style="background-color:#157347;">
                            <i class="bi bi-tools me-1"></i>ขายพร้อมติดตั้ง
                        </th>
                        <!-- Totals -->
                        <th class="text-center" colspan="2" style="background-color:#343a40;">
                            <i class="bi bi-sigma me-1"></i>รวมทั้งหมด
                        </th>
                        <th class="text-center" style="width:100px">จัดการ</th>
                    </tr>
                    <tr class="small" style="font-size:.78rem">
                        <th></th>
                        <th></th>
                        <th></th>
                        <th class="text-center text-info" style="background-color:#0b5ed7;">จำนวน (ชิ้น)</th>
                        <th class="text-center text-info" style="background-color:#0b5ed7;">ยอดขาย (บาท)</th>
                        <th class="text-center" style="background-color:#157347;color:#a3e6b9;">จำนวน (ชิ้น)</th>
                        <th class="text-center" style="background-color:#157347;color:#a3e6b9;">ยอดขาย (บาท)</th>
                        <th class="text-center text-warning-emphasis" style="background-color:#343a40;">จำนวน (ชิ้น)</th>
                        <th class="text-center text-warning-emphasis" style="background-color:#343a40;">ยอดขาย (บาท)</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($products as $idx => $product): ?>
                    <tr>
                        <!-- # -->
                        <td class="text-center text-muted small">
                            <?= $idx + 1 ?>
                        </td>

                        <!-- Product name -->
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge bg-light text-dark border rounded-circle"
                                      style="width:32px;height:32px;display:inline-flex!important;align-items:center;justify-content:center;font-size:.8rem;font-weight:700;">
                                    <?= mb_substr(e($product['name']), 0, 1) ?>
                                </span>
                                <div>
                                    <div class="fw-semibold"><?= e($product['name']) ?></div>
                                    <div class="text-muted" style="font-size:.75rem">
                                        เพิ่มเมื่อ <?= formatDate($product['created_at']) ?>
                                    </div>
                                </div>
                            </div>
                        </td>

                        <!-- Category -->
                        <td>
                            <?php if (!empty($product['category_name'])): ?>
                                <span class="badge bg-info text-dark">
                                    <i class="bi bi-tag-fill me-1"></i><?= e($product['category_name']) ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted small">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- Work type 1: qty -->
                        <td class="text-center">
                            <?php if ((int)$product['wt1_qty'] > 0): ?>
                                <span class="badge badge-wt1 rounded-pill text-amount">
                                    <?= number_format((int)$product['wt1_qty']) ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted small">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- Work type 1: amount -->
                        <td class="text-end">
                            <?php if ((float)$product['wt1_amount'] > 0): ?>
                                <span class="fw-semibold text-primary text-amount">
                                    <?= formatMoney($product['wt1_amount']) ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted small">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- Work type 2: qty -->
                        <td class="text-center">
                            <?php if ((int)$product['wt2_qty'] > 0): ?>
                                <span class="badge badge-wt2 rounded-pill text-amount">
                                    <?= number_format((int)$product['wt2_qty']) ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted small">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- Work type 2: amount -->
                        <td class="text-end">
                            <?php if ((float)$product['wt2_amount'] > 0): ?>
                                <span class="fw-semibold text-success text-amount">
                                    <?= formatMoney($product['wt2_amount']) ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted small">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- Total qty -->
                        <td class="text-center">
                            <?php if ((int)$product['total_qty'] > 0): ?>
                                <span class="fw-bold text-amount">
                                    <?= number_format((int)$product['total_qty']) ?>
                                </span>
                                <span class="text-muted small"> ชิ้น</span>
                            <?php else: ?>
                                <span class="text-muted small">ยังไม่มีการขาย</span>
                            <?php endif; ?>
                        </td>

                        <!-- Total amount -->
                        <td class="text-end">
                            <?php if ((float)$product['total_amount'] > 0): ?>
                                <span class="fw-bold text-dark text-amount">
                                    <?= formatMoney($product['total_amount']) ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted small">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- Actions -->
                        <td class="text-center">
                            <div class="d-flex gap-1 justify-content-center">
                                <!-- View sales for this product -->
                                <a href="<?= url(['page' => 'sales', 'product_id' => $product['id'], 'month' => date('n'), 'year' => date('Y')]) ?>"
                                   class="btn btn-sm btn-outline-secondary"
                                   data-bs-toggle="tooltip"
                                   title="ดูรายการขาย">
                                    <i class="bi bi-eye-fill"></i>
                                </a>

                                <!-- Edit -->
                                <a href="<?= url(['page' => 'products', 'action' => 'edit', 'id' => $product['id']]) ?>"
                                   class="btn btn-sm btn-outline-primary"
                                   data-bs-toggle="tooltip"
                                   title="แก้ไข">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>

                                <!-- Delete -->
                                <form method="POST"
                                      action="<?= url(['page' => 'products', 'action' => 'delete']) ?>"
                                      class="form-delete d-inline"
                                      data-confirm="ต้องการลบสินค้า «<?= e($product['name']) ?>» ออกจากระบบใช่หรือไม่?">
                                    <?php csrfField(); ?>
                                    <input type="hidden" name="id" value="<?= $product['id'] ?>">
                                    <button type="submit"
                                            class="btn btn-sm btn-outline-danger"
                                            data-bs-toggle="tooltip"
                                            title="ลบ"
                                            <?= ((int)$product['total_qty'] > 0) ? 'disabled data-bs-toggle="tooltip" title="ไม่สามารถลบได้ เนื่องจากมีรายการขาย"' : '' ?>>
                                        <i class="bi bi-trash-fill"></i>
                                    </button>
                                </form>
                            </div>
                        </td>
                    </tr>

                    <?php endforeach; ?>
                </tbody>

                <!-- Grand total footer -->
                <tfoot class="table-secondary fw-bold">
                    <tr>
                        <td colspan="2" class="text-end text-muted">
                            รวมทั้งหมด (<?= number_format(count($products)) ?> สินค้า):
                        </td>
                        <td class="text-center text-amount">
                            <?= number_format($overallWt1Qty) ?>
                        </td>
                        <td class="text-end text-amount text-primary">
                            <?= formatMoney($overallWt1Amount) ?>
                        </td>
                        <td class="text-center text-amount">
                            <?= number_format($overallWt2Qty) ?>
                        </td>
                        <td class="text-end text-amount text-success">
                            <?= formatMoney($overallWt2Amount) ?>
                        </td>
                        <td class="text-center text-amount">
                            <?= number_format($overallTotalQty) ?> ชิ้น
                        </td>
                        <td class="text-end text-amount">
                            <?= formatMoney($overallTotalAmount) ?>
                        </td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
        </div>

        <!-- ── Visual breakdown bar ───────────────────────────────── -->
        <?php if ($overallTotalAmount > 0): ?>
        <div class="card-footer bg-white px-3 py-3">
            <p class="small fw-semibold text-muted mb-2">
                <i class="bi bi-bar-chart-fill me-1"></i>สัดส่วนยอดขายตามประเภทงาน (ทุกสินค้า)
            </p>
            <?php
            $wt1Pct = round(($overallWt1Amount / $overallTotalAmount) * 100, 1);
            $wt2Pct = round(($overallWt2Amount / $overallTotalAmount) * 100, 1);
            ?>
            <div class="progress mb-2" style="height:22px; border-radius:.5rem;">
                <?php if ($wt1Pct > 0): ?>
                <div class="progress-bar badge-wt1"
                     role="progressbar"
                     style="width:<?= $wt1Pct ?>%"
                     title="ขายเฉพาะเครื่อง <?= $wt1Pct ?>%">
                    <?php if ($wt1Pct >= 10): ?>
                        <span class="small"><?= $wt1Pct ?>%</span>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
                <?php if ($wt2Pct > 0): ?>
                <div class="progress-bar badge-wt2"
                     role="progressbar"
                     style="width:<?= $wt2Pct ?>%"
                     title="ขายพร้อมติดตั้ง <?= $wt2Pct ?>%">
                    <?php if ($wt2Pct >= 10): ?>
                        <span class="small"><?= $wt2Pct ?>%</span>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
            <div class="d-flex gap-4 flex-wrap small text-muted">
                <span>
                    <span class="badge badge-wt1 me-1">&nbsp;</span>
                    ขายเฉพาะเครื่อง:
                    <strong class="text-dark">฿<?= formatMoney($overallWt1Amount) ?></strong>
                    (<?= $wt1Pct ?>%)
                </span>
                <span>
                    <span class="badge badge-wt2 me-1">&nbsp;</span>
                    ขายพร้อมติดตั้ง:
                    <strong class="text-dark">฿<?= formatMoney($overallWt2Amount) ?></strong>
                    (<?= $wt2Pct ?>%)
                </span>
            </div>
        </div>
        <?php endif; ?>

    <?php endif; ?>
</div>

<!-- ── Per-product breakdown (expandable) ──────────────────── -->
<?php if (!empty($products)): ?>
<div class="row g-3 mt-1">
    <?php foreach ($products as $product): ?>
    <?php if ((float)$product['total_amount'] > 0): ?>
    <div class="col-12 col-md-6 col-xl-4">
        <div class="card h-100">
            <div class="card-header bg-white py-2 d-flex align-items-center justify-content-between">
                <span class="fw-semibold">
                    <i class="bi bi-box-seam me-1 text-primary"></i>
                    <?= e($product['name']) ?>
                </span>
                <a href="<?= url(['page' => 'products', 'action' => 'edit', 'id' => $product['id']]) ?>"
                   class="btn btn-xs btn-outline-secondary btn-sm py-0 px-2">
                    <i class="bi bi-pencil"></i>
                </a>
            </div>
            <div class="card-body pb-2">

                <!-- Mini stats row -->
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        <div class="text-center bg-light rounded p-2">
                            <div class="small text-muted">รวมชิ้น</div>
                            <div class="fw-bold text-amount fs-5"><?= number_format((int)$product['total_qty']) ?></div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="text-center bg-light rounded p-2">
                            <div class="small text-muted">รวมยอด (฿)</div>
                            <div class="fw-bold text-amount fs-6"><?= formatMoney($product['total_amount']) ?></div>
                        </div>
                    </div>
                </div>

                <!-- Work type breakdown -->
                <table class="table table-sm mb-0 small">
                    <thead>
                        <tr class="table-light">
                            <th>ประเภทงาน</th>
                            <th class="text-center">จำนวน</th>
                            <th class="text-end">ยอดขาย</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>
                                <span class="badge badge-wt1 rounded-pill me-1">1</span>
                                ขายเฉพาะเครื่อง
                            </td>
                            <td class="text-center text-amount">
                                <?= number_format((int)$product['wt1_qty']) ?> ชิ้น
                            </td>
                            <td class="text-end text-amount text-primary">
                                <?= formatMoney($product['wt1_amount']) ?>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <span class="badge badge-wt2 rounded-pill me-1">2</span>
                                ขายพร้อมติดตั้ง
                            </td>
                            <td class="text-center text-amount">
                                <?= number_format((int)$product['wt2_qty']) ?> ชิ้น
                            </td>
                            <td class="text-end text-amount text-success">
                                <?= formatMoney($product['wt2_amount']) ?>
                            </td>
                        </tr>
                    </tbody>
                </table>

                <!-- Mini progress bar -->
                <?php
                $pTotalAmt = (float)$product['total_amount'];
                $pWt1Pct   = $pTotalAmt > 0 ? round(((float)$product['wt1_amount'] / $pTotalAmt) * 100, 1) : 0;
                $pWt2Pct   = $pTotalAmt > 0 ? round(((float)$product['wt2_amount'] / $pTotalAmt) * 100, 1) : 0;
                ?>
                <div class="progress mt-2" style="height:8px;border-radius:4px;">
                    <?php if ($pWt1Pct > 0): ?>
                    <div class="progress-bar badge-wt1"
                         role="progressbar"
                         style="width:<?= $pWt1Pct ?>%"
                         title="ขายเฉพาะเครื่อง <?= $pWt1Pct ?>%">
                    </div>
                    <?php endif; ?>
                    <?php if ($pWt2Pct > 0): ?>
                    <div class="progress-bar badge-wt2"
                         role="progressbar"
                         style="width:<?= $pWt2Pct ?>%"
                         title="ขายพร้อมติดตั้ง <?= $pWt2Pct ?>%">
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            <div class="card-footer bg-white py-2 text-center">
                <a href="<?= url(['page' => 'sales', 'product_id' => $product['id'], 'month' => date('n'), 'year' => date('Y')]) ?>"
                   class="btn btn-sm btn-outline-primary w-100">
                    <i class="bi bi-receipt me-1"></i>ดูรายการขาย
                </a>
            </div>
        </div>
    </div>
    <?php endif; ?>
    <?php endforeach; ?>
</div>
<?php endif; ?>
